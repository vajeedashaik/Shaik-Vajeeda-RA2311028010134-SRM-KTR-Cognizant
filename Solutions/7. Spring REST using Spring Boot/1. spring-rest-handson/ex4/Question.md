...
1. Pick any one of your choice country to configure in Spring XML configuration named country.xml.
2. Create a bean tag in spring configuration for country and set the property and values
...
3. Create Country class with following aspects:
...
4. Create a method displayCountry() in SpringLearnApplication.java, which will read the country bean from spring configuration file and display the country details. ClassPathXmlApplicationContext, ApplicationContext and context.getBean("beanId", Country.class). Refer sample code for displayCountry() method below.
...
5. Invoke displayCountry() method in main() method of SpringLearnApplication.java.
6. Execute main() method and check the logs to find out which constructors and methods were invoked.
